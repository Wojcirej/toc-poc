class S3Adapter {
  ...
  async createBucket(bucketName, region = "eu-west-1") {
    const params = {
      Bucket: bucketName,
  ...
  async deleteBucket(bucketName) {
    ...
    try {
      return !!(await this.s3Service.deleteBucket(params).promise());
    } catch (error) {
      return false;
    }
  }
  ...
  async deleteObject(bucketName, key) {
    ...
    try {
      return !!(await this.s3Service.deleteObject(params).promise());
    } catch (error) {
      return false;
    }
  }
